            ...
            <form [formGroup]="employeeForm" (ngSubmit)="onSubmitEmployee()">
              ...
              <div class="form-group" style="margin-bottom: 1rem;">
                ...
                <div *ngIf="employeeForm.get('phone')?.touched && employeeForm.get('phone')?.invalid" class="validation-error" style="color: var(--color-error); font-size: 0.75rem; margin-top: 4px;">
                  ...
                  <span *ngIf="employeeForm.get('phone')?.hasError('minlength') || employeeForm.get('phone')?.hasError('maxlength')">
                    Phone number must be exactly {{ selectedCountry?.mobileNumberLength }} digits for {{ selectedCountry?.countryName }}.
                  </span>
                  <span *ngIf="employeeForm.get('phone')?.hasError('pattern')">Only numeric digits allowed.</span>
                </div>
              </div>
